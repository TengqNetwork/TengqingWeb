...
Vue.component('pageination', {
    ...
    methods: {
        ...
        pagers:function() {
            ...
            //开始页码1
             start = this.pageinationCurrentPage - 4 > 1 ? this.pageinationCurrentPage - 4 : 1;
            //当前页码减去开始页码得到差
             interval = this.pageinationCurrentPage - start;
            //最多9个页码，总页码减去interval 得到end要显示的数量+
             end = (5 - interval) < this.pageinationPage ? (5 - interval) : this.pageinationPage;
            //最末页码减开始页码小于8
            if ((end - start) != 4) {
                //最末页码加上与不足9页的数量，数量不得大于总页数
                end = end + (4 - (end - start)) < this.pageinationPage ? end + (4 - (end - start)) : this.pageinationPage;
